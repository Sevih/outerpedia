...
export default function TranscendenceSlider({ transcendData }: Props) {
  const steps = useMemo(() => {
    ...
    if (transcendData["4"]) s.push({ label: "Lv 4", key: "4" }); //4star 1 et deux etoile
    if (transcendData["4_1"]) s.push({ label: "Lv 4", key: "4_1" });
    if (transcendData["4_2"]) s.push({ label: "Lv 4+", key: "4_2" });
    if (transcendData["5"]) s.push({ label: "Lv 5", key: "5" });//5star 1 et deux etoile
  ...
  return (<div className="bg-[#1a1c28] border border-gray-700 p-4 space-y-3 text-white max-w-[400px] w-full">
    {/* Ligne slider complète */}
    <div className="flex items-center justify-between gap-3">
      {/* Contrôles - Slider - Étoiles */}
    ...
    <div className="space-y-1.5">
      {activeBonuses.map((bonus, idx) => {
        ...
        return (
          <div
            key={idx}
            className="text-xs text-white whitespace-pre-line leading-tight"
          >
            {bonus}
          </div>
        );
