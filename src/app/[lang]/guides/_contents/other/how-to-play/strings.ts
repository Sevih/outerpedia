...
export const S = {
  ...
  steamDesc: {
    en: 'Play Outerplane on PC through its official Steam release for Windows. Free to play, on a larger screen with mouse and keyboard support.',
    jp: 'Windows向けのSteam公式版で、PCからOuterplaneをプレイできます。基本プレイ無料で、大画面でマウスとキーボード操作が可能です。',
    kr: 'Windows용 Steam 공식 버전으로 PC에서 Outerplane을 플레이할 수 있습니다. 무료로 플레이할 수 있으며, 큰 화면에서 마우스와 키보드를 사용할 수 있습니다.',
    zh: '通过官方Steam版本（Windows）在电脑上游玩异域战记：免费游玩，享受大屏游戏与键鼠支持。',
    fr: 'Jouez à Outerplane sur PC grâce à sa version Steam officielle pour Windows. Gratuit, sur un plus grand écran, avec la souris et le clavier.',
    es: 'Juega a Outerplane en PC con su versión oficial de Steam para Windows. Gratis, en una pantalla más grande y con soporte de mouse y teclado.',
  },
  steamBtn: {
    en: 'View on Steam',
    jp: 'Steamで見る',
  ...
  gpgDesc: {
    en: 'Alternatively, play the mobile version on PC with Google Play Games for PC (Beta), the official PC platform supported by Google, with mouse and keyboard support.',
    jp: 'または、Googleが公式にサポートするPC版プラットフォームGoogle Play Games for PC（ベータ版）で、モバイル版をPCでプレイすることもできます。マウスとキーボード操作に対応しています。',
    kr: '또는 Google이 공식적으로 지원하는 PC 플랫폼인 Google Play Games for PC (베타)를 사용해 모바일 버전을 PC에서 플레이할 수도 있습니다. 마우스와 키보드를 지원합니다.',
    zh: '您也可以通过受谷歌支持的官方电脑端——Google Play游戏电脑版——在电脑上游玩移动版，同样支持键鼠操作。',
    fr: 'Autre option : jouez à la version mobile sur PC avec Google Play Games pour PC (Bêta), la plateforme PC officielle prise en charge par Google, avec la souris et le clavier.',
    es: 'Como alternativa, juega a la versión móvil en PC usando Google Play Games para PC (Beta), la plataforma oficial de PC compatible con Google, con soporte de mouse y teclado.',
  },
  gpgBtn: {
  ...
  pcWarning: {
    en: 'For the best experience and official support, use one of the official platforms above: Steam or Google Play Games for PC. While other emulators may work, they are not officially supported and we cannot guarantee their stability or security.',
    jp: '最高の体験と公式サポートのため、上記の公式プラットフォーム（SteamまたはGoogle Play Games for PC）をご利用ください。他のエミュレーターでも動作する可能性はありますが、公式にはサポートされておらず、安定性やセキュリティを保証できません。',
    kr: '최상의 경험과 공식 지원을 위해 위의 공식 플랫폼(Steam 또는 Google Play Games for PC)을 이용하세요. 다른 에뮬레이터도 작동할 수 있지만 공식적으로 지원되지 않으며 안정성이나 보안을 보장할 수 없습니다.',
    zh: '我们建议使用上述官方平台（Steam或Google Play游戏电脑版）以获得最佳游戏体验与官方支持。别的模拟器也许可以运行，但它们不受官方支持，且我们也不能保证它们的稳定性与安全性。',
    fr: "pour une expérience optimale et un support officiel, utilisez l'une des plateformes officielles ci-dessus : Steam ou Google Play Games pour PC. Bien que d'autres émulateurs puissent fonctionner, ils ne sont pas officiellement pris en charge et nous ne pouvons garantir leur stabilité ni leur sécurité.",
    es: 'Para la mejor experiencia y soporte oficial, usa una de las plataformas oficiales de arriba: Steam o Google Play Games para PC. Aunque otros emuladores pueden funcionar, no cuentan con soporte oficial y no podemos garantizar su estabilidad o seguridad.',
  },
  startTitle: {
    en: 'Getting Started',
    jp: 'はじめに',
...
export const LINKS = {
  ...
  steam: 'https://store.steampowered.com/app/4247320/OUTERPLANE/',
