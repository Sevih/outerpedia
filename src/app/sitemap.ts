...
export const dynamic = 'force-dynamic'  // ⬅️ change ceci
...
function statMtime(p: string) { try { return fs.statSync(p).mtime } catch { return undefined } }
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    // Static
    {
        ...
        for (const p of STATIC_PAGES) {
            urls.push({ url: base(p), lastModified: mtime, changeFrequency: 'daily', priority: p === '/' ? 1.0 : 0.7 })
        }
    }
