...
export function breadcrumbLd(
  ...
  const base = normalizeBase(domain)
  const currentUrl = `${base}${labels.currentPath ?? '/tierlistpve'}`
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
...
export function tierItemListLd(
  domain: string,
  opts: {
    title: string
    description: string
    characters: { slug: string; id: string; name: string }[]
  }
): JsonLdObject {
  const base = normalizeBase(domain)
  return {
    ...
    itemListElement: opts.characters.map((c, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      url: `${base}/characters/${c.slug}`,
      name: c.name,
      image: `${base}/images/characters/portrait/CT_${c.id}.webp`,
    })),
  }
}
