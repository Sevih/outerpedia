'use client'

import { useState, useCallback } from 'react'
import GuideTemplate from '@/app/components/GuideTemplate'
import StageBasedTeamSelector from '@/app/components/StageBasedTeamSelector'
import CombatFootage from '@/app/components/CombatFootage'
import BossDisplay from '@/app/components/BossDisplay'
import MiniBossDisplay from '@/app/components/MiniBossDisplay'
import SacreedTeamsData from './Sacreed.json'
import type { TeamData } from '@/types/team'
import TacticalTips from '@/app/components/TacticalTips'
import RecommendedCharacterList from '@/app/components/RecommendedCharacterList'
import { recommendedCharacters } from './recommendedCharacters'

const SacreedTeams = SacreedTeamsData as Record<string, TeamData>

export default function SacreedGuardian13Guide() {
    const [miniBossId, setMiniBossId] = useState('404400462')

    // Mapping: Sacreed Guardian boss ID → Deformed Inferior Core boss ID
    const handleBossChange = useCallback((sacreedBossId: string) => {
        const bossIdMap: Record<string, string> = {
            '404400362': '404400462', // Stage 13
            '404400361': '404400461', // Stage 12
            '404400360': '404400460', // Stage 11
            '404400359': '404400459', // Stage 10
        }
        // Stage 9 et moins utilisent tous 404400450
        setMiniBossId(bossIdMap[sacreedBossId] || '404400450')
    }, [])

    return (
        <GuideTemplate
            title="サクリードガーディアン特殊依頼攻略ガイド"
            introduction="サクリードガーディアンは{D/BT_SEALED}が無効で、{D/BT_REMOVE_BUFF}、{D/BT_STEAL_BUFF}、または{D/BT_STATBUFF_CONVERT_TO_STATDEBUFF}が必要です。ボスがバフを持った状態でターンを迎えると、チームをスタンさせ無敵になります。オーブは毎ターン3つのバフを付与するため、制御が必要です。"
            defaultVersion="default"
            versions={{
                default: {
                    label: 'ガイド',
                    content: (
                        <>
                            <BossDisplay
                                bossKey='Sacreed Guardian'
                                modeKey='Special Request: Ecology Study'
                                defaultBossId='404400362'
                                onBossChange={handleBossChange}
                            />
                            <MiniBossDisplay
                                bosses={[
                                    { bossKey: 'Deformed Inferior Core', defaultBossId: miniBossId }
                                ]}
                                modeKey='Special Request: Ecology Study'
                            />
                            <hr className="my-6 border-neutral-700" />
                            <TacticalTips tips={[
                                "{D/BT_SEALED}無効。代わりに{D/BT_STEAL_BUFF}、{D/BT_STATBUFF_CONVERT_TO_STATDEBUFF}、または{D/BT_REMOVE_BUFF}を使用してください。",
                                "ボスがバフを持った状態でターンを迎えると、チームに{D/BT_STUN}を付与し{B/BT_INVINCIBLE}を獲得します。",
                                "戦闘開始時にボスの{B/BT_STAT|ST_SPEED}バフを素早いユニットで除去しましょう。",
                                "ステージ11まで{C/Healer}ユニットはスタンされません。",
                                "回復は任意です。ボスがバフを獲得する前にロックダウンとバーストに集中しましょう。",
                                "ステージ12：{B/SYS_BUFF_REVENGE}、{B/BT_RUN_PASSIVE_SKILL_ON_TURN_END_DEFENDER_NO_CHECK}、{B/BT_STAT|ST_COUNTER_RATE}が発動するとボスが回復します。",
                                "ステージ13：ボスは{B/SYS_BUFF_REVENGE}、{B/BT_RUN_PASSIVE_SKILL_ON_TURN_END_DEFENDER_NO_CHECK}、{B/BT_STAT|ST_COUNTER_RATE}を無効化します。"
                            ]} />
                            <hr className="my-6 border-neutral-700" />
                            <RecommendedCharacterList entries={recommendedCharacters} />
                            <hr className="my-6 border-neutral-700" />
                            <StageBasedTeamSelector teamData={SacreedTeams.sacreedSpecialRequest} defaultStage="1-10" />
                            <hr className="my-6 border-neutral-700" />
                            <CombatFootage videoId="fLdbR9Sa7G0" title="サクリードガーディアン13 – クリアラン映像" author="Sevih" date="09/05/2025" />
                        </>
                    ),
                },
            }}
        />
    )
}
