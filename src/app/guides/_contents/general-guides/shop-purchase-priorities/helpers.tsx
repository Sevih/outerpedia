...
// ---- Tabs config (icons only, labels from i18n)
export const SHOP_TAB_CONFIG: { key: ShopKey; i18nKey: string; icon: string }[] = [
    { key: 'guild', i18nKey: 'progress.shop.guild-shop', icon: '/images/ui/shop_guild.webp' },
    { key: 'supply', i18nKey: 'progress.shop.supply-module', icon: '/images/ui/shop_supply.webp' },
    { key: 'rico', i18nKey: 'progress.shop.rico-secret', icon: '/images/ui/shop_rico.webp' },
    { key: 'event', i18nKey: 'progress.shop.event-shop', icon: '/images/ui/shop_event.webp' },
    { key: 'joint', i18nKey: 'progress.shop.joint-challenge', icon: '/images/ui/shop_joint.webp' },
...
export function getTabs(t: (key: string) => string): { key: ShopKey; label: string; icon: string }[] {
    return SHOP_TAB_CONFIG.map(tab => ({
        key: tab.key,
        label: t(tab.i18nKey),
        icon: tab.icon
    }))
...
export const textOnlyShopsContent: Partial<Record<ShopKey, TextOnlyShopContent>> = {
    supply: {
        paragraphs: [
            {
                en: "Only worth buying {Basic Skill Manual} and {Intermediate Skill Manual} for {Antimatter}.",
                jp: "{Basic Skill Manual}と{Intermediate Skill Manual}を{Antimatter}で購入する価値があります。",
                kr: "{Basic Skill Manual}과 {Intermediate Skill Manual}을 {Antimatter}로 구매할 가치가 있습니다.",
                zh: "只值得用{Antimatter}购买{Basic Skill Manual}和{Intermediate Skill Manual}。"
            },
            {
                en: "{Intermediate Skill Manual} for {Free Ether} is fine if discounted and you really need them.",
                jp: "割引があり本当に必要な場合、{Free Ether}で{Intermediate Skill Manual}を購入しても構いません。",
                kr: "할인이 있고 정말 필요하다면 {Free Ether}로 {Intermediate Skill Manual}을 구매해도 괜찮습니다.",
                zh: "如果有折扣且确实需要，用{Free Ether}购买{Intermediate Skill Manual}也可以。"
            }
        ],
        gearNote: {
    ...
    rico: {
        paragraphs: [
            {
                en: "Always buy {Special Recruitment Ticket (Event)} when it appears for {Gold}.",
                jp: "{Gold}で{Special Recruitment Ticket (Event)}が出たら必ず購入しましょう。",
                kr: "{Gold}로 {Special Recruitment Ticket (Event)}가 나오면 반드시 구매하세요.",
                zh: "当{Special Recruitment Ticket (Event)}以{Gold}出售时务必购买。"
            },
            {
                en: "Purchasing {Special Recruitment Ticket} with {Free Ether} is ill-advised due to expenses that come from Premium/Limited banners and Precise crafting.",
                jp: "{Free Ether}で{Special Recruitment Ticket}を購入するのは、プレミアム/限定ガチャや精密クラフトの出費があるためお勧めしません。",
                kr: "{Free Ether}로 {Special Recruitment Ticket}을 구매하는 것은 프리미엄/한정 배너와 정밀 제작에 필요한 비용 때문에 권장하지 않습니다.",
                zh: "用{Free Ether}购买{Special Recruitment Ticket}不明智，因为需要用于限定池和精准制作的开支。"
            },
            {
                en: "{Potentium (Armor)} / {Potentium (Weapon/Accessory)} are ok if the discount is 25–30%.",
                jp: "{Potentium (Armor)} / {Potentium (Weapon/Accessory)}は25〜30%の割引があれば購入しても良いでしょう。",
                kr: "{Potentium (Armor)} / {Potentium (Weapon/Accessory)}는 25~30% 할인이 있다면 구매해도 괜찮습니다.",
                zh: "如果{Potentium (Armor)} / {Potentium (Weapon/Accessory)}有25-30%的折扣可以购买。"
            }
        ],
        gearNote: {
...
export function ShopTextSection({ children }: { children: ReactNode }) {
    return (
...
export function GearUsageLink({ children }: { children: ReactNode }) {
    return (
        <Link
            href="/gear-solver"
...
// ---- Main Components
export function ShopTable({ items, lang }: { items: ShopItem[]; lang: TenantKey }) {
            ...
            <div className="w-full max-w-4xl">
                <table className="w-full border border-gray-700 rounded-md text-sm">
                    <thead className="bg-gray-800">
                        <tr>
                            <th className="border px-3 py-2 text-left">{lRec(LABELS.colPriority, lang)}</th>
                            <th className="border px-3 py-2 text-left">{lRec(LABELS.colItem, lang)}</th>
                            <th className="border px-3 py-2 text-left">{lRec(LABELS.colGives, lang)}</th>
                            <th className="border px-3 py-2">{lRec(LABELS.colCost, lang)}</th>
                            <th className="border px-3 py-2">{lRec(LABELS.colLimit, lang)}</th>
                            {hasNotes && <th className="border px-3 py-2 text-left">{lRec(LABELS.colNotes, lang)}</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {sorted.map((it, i) => (
...
// Parse text with {ItemName} placeholders and render ItemInlineDisplay
function parseTextWithItems(text: string): ReactNode[] {
    const parts = text.split(/(\{[^}]+\})/)
    return parts.map((part, i) => {
        const match = part.match(/^\{(.+)\}$/)
        if (match) {
            return <ItemInlineDisplay key={i} names={[match[1]]} />
        }
        return part
    })
}

export function TextOnlyShopSection({ shopKey, lang }: { shopKey: ShopKey; lang: TenantKey }) {
    ...
    return (
        <ShopTextSection>
            {content.paragraphs.map((p, i) => (
                <p key={i}>{parseTextWithItems(lRec(p, lang))}</p>
            ))}
            {content.gearNote && (
                <p>
