...
// Image base path
const IMG_BASE = "/images/guides/general-guides/roadmap-2026";
...
export interface MonthlyUpdate {
    ...
    coreFusions?: string[]; // Character names, not translated
    balance?: LangMap[];
    content?: LangMap[];
}

export interface NewCharacterData {
    name: string; // Character names not translated
    element: ElementType;
    classType: ClassType;
    borderColor: string;
    bgColor: string;
...
export const DEMIURGE_LIMITED_PLANS: { label: LangMap; text: LangMap }[] = [
    ...
    {
        label: { en: "Seasonal Chars BokGak:", jp: "季節キャラ復刻:", kr: "시즌 캐릭터 복각:", zh: "季节角色复刻：" },
...
export function QuarterCard({ data, lang }: { data: RoadmapQuarter; lang: TenantKey }) {
    return (
...
export function MonthCard({ data, lang }: { data: MonthlyUpdate; lang: TenantKey }) {
    return (
...
export function NewCharacterCard({ data, lang }: { data: NewCharacterData; lang: TenantKey }) {
        ...
        <div className={`rounded-lg border ${data.borderColor} ${data.bgColor} p-4 flex gap-4`}>
            <div className="flex-1">
                ...
                <p className="text-sm text-gray-300 mb-2">
                    <ElementInlineTag element={data.element} /> <ClassInlineTag name={data.classType} />
                </p>
                {pveText || pvpText ? (
                    <ul className="text-xs text-gray-400 space-y-1">
...
export function CoreFusionSection({ lang }: { lang: TenantKey }) {
    return (
        <>
            ...
            <div className="flex flex-wrap gap-4 justify-center">
                {CORE_FUSION_CHARS.map((cf) => (
                    <Link
                        key={cf.id}
                        href={`/characters/${cf.name.toLowerCase()}`}
                        className="flex flex-col items-center gap-1 hover:scale-105 transition-transform"
                    >
                        <CharacterPortrait
                            characterId={cf.id}
                            characterName={cf.name}
                            size={70}
...
export function DimensionSingularitySection({ lang }: { lang: TenantKey }) {
    return (
        <>
            <div className="flex justify-center mb-4">
                <ImageLightbox
...
export function RTASection({ lang }: { lang: TenantKey }) {
    return (
        <>
            <div className="flex justify-center mb-4">
                <ImageLightbox
...
export function CouponSection({ lang }: { lang: TenantKey }) {
    return (
        <div className="p-4 rounded-lg bg-gradient-to-r from-pink-900/30 to-red-900/30 border border-pink-700/50">
...
export function QuarterlyRoadmapSection({ lang }: { lang: TenantKey }) {
    return (
        <>
            <div className="flex justify-center mb-4">
                <ImageLightbox
...
export function MonthlyUpdatesSection({ lang }: { lang: TenantKey }) {
    return (
        <>
...
export function NewCharactersSection({ lang }: { lang: TenantKey }) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
export function DemiurgeLimitedPlansSection({ lang }: { lang: TenantKey }) {
    return (
        <ul className="list-disc pl-6 space-y-2 text-gray-300">
...
export function DevelopmentDirectionSection({ lang }: { lang: TenantKey }) {
    return (
        <ul className="list-disc pl-6 space-y-2 text-gray-300">
...
export function CreditsSection({ lang }: { lang: TenantKey }) {
    return (
