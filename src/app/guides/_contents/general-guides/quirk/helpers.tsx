...
function parseWithIcons(text: string, lang: TenantKey, size: number = 25): ReactNode[] {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        // Text before the match - parse with parseText for inline tags
        if (index > lastIndex) {
        ...
        if (icon) {
            ...
            parts.push(
                <GuideIconInline key={`icon-${index}`} name={icon.name} text={localizedText} size={size} />
            )
        } else {
            parts.push(full)
        }
...
export const LABELS = {
    ...
    exampleP1: {
        en: "After unlocking boss quirks, prioritize the {C/Striker} tree — especially the left path, which benefits Attackers like {P-I/Aer} and {P-I/Drakhan}.",
        jp: "ボスギフトを解放したら、{C/Striker}ツリーを優先しましょう。特に左側のパスは{P-I/Aer}や{P-I/Drakhan}のようなアタッカーに有利です。",
        kr: "보스 기프트를 해금한 후 {C/Striker} 트리를 우선하세요. 특히 왼쪽 경로는 {P-I/Aer}와 {P-I/Drakhan} 같은 어태커에게 유리합니다.",
        zh: "解锁Boss天赋后，优先{C/Striker}树——特别是左侧路径，对{P-I/Aer}和{P-I/Drakhan}等攻击者有利。",
    },
    exampleP2: {
        en: "Then, upgrade the {E/Fire} tree ({P-I/Valentine} and {P-I/Aer}) — one of your early goals will be farming Chimera for Speed gear.",
        jp: "次に、{E/Fire}ツリーをアップグレードしましょう（{P-I/Valentine}と{P-I/Aer}）。序盤の目標の一つはキメラでスピード装備を集めることです。",
        kr: "그 다음, {E/Fire} 트리를 업그레이드하세요 ({P-I/Valentine}와 {P-I/Aer}). 초반 목표 중 하나는 키메라에서 스피드 장비를 파밍하는 것입니다.",
        zh: "然后升级{E/Fire}树（{P-I/Valentine}和{P-I/Aer}）——前期目标之一是刷奇美拉获取速度装备。",
    },
    exampleP3: {
        en: "Then, invest in {E/Light} quirks for {P-I/Drakhan} and {P-I/Monad Eva}.",
        jp: "その後、{P-I/Drakhan}と{P-I/Monad Eva}のために{E/Light}ギフトに投資しましょう。",
        kr: "그 다음, {P-I/Drakhan}와 {P-I/Monad Eva}를 위해 {E/Light} 기프트에 투자하세요.",
        zh: "然后为{P-I/Drakhan}和{P-I/Monad Eva}投资{E/Light}天赋。",
    },
    exampleOutro: {
...
/* ===================== FAQ Data ===================== */
export function getQuirkFaq(lang: TenantKey): FAQItem[] {
    return [
        {
            key: "quirk-respec",
            title: lRec(LABELS.faqRespecTitle, lang),
            content: <p>{parseText(lRec(LABELS.faqRespecContent, lang))}</p>,
        },
        {
            key: "quirk-subnodes",
            title: lRec(LABELS.faqSubnodesTitle, lang),
            content: (
                <>
                    <p>{lRec(LABELS.faqSubnodesP1, lang)}</p>
                    <p className="mt-2">{parseWithIcons(lRec(LABELS.faqSubnodesP2, lang), lang, 24)}</p>
                </>
            ),
        },
        {
            key: "quirk-skip-nodes",
            title: lRec(LABELS.faqSkipTitle, lang),
            content: (
                <>
        ...
        },
        {
            ...
            title: lRec(LABELS.faqMaterialsTitle, lang),
            content: (
                <ul className="list-disc list-inside ml-4">
...
export function IntroSection({ lang }: QuirkGuideContentProps) {
    return (
        <p>
            {lRec(LABELS.introP1, lang)}<br />
            {parseText(lRec(LABELS.introP2, lang))}
        </p>
    )
}
...
export function CategoryOverviewSection({ lang }: QuirkGuideContentProps) {
    return (
        <ul className="list-inside ml-4 space-y-1">
            <li><GuideIconInline name="CM_Gift_Menu_05" text={lRec(TERMS.counteractStrongEnemies, lang)} size={40} />: {lRec(LABELS.catCounteract, lang)}</li>
            <li><GuideIconInline name="CM_Gift_Menu_03" text={lRec(TERMS.classEnhancement, lang)} size={40} />: {lRec(LABELS.catClass, lang)}</li>
            <li><GuideIconInline name="CM_Gift_Menu_01" text={lRec(TERMS.elementEnhancement, lang)} size={40} />: {lRec(LABELS.catElement, lang)}</li>
            <li><GuideIconInline name="CM_Gift_Menu_04" text={lRec(TERMS.utility, lang)} size={40} />: {lRec(LABELS.catUtility, lang)}</li>
            <li><GuideIconInline name="CM_Gift_Menu_06" text={lRec(TERMS.adventureLicense, lang)} size={40} />: {lRec(LABELS.catAdventure, lang)}</li>
        </ul>
    )
}
