...
// ---- Static Data (shared across all languages)
...
export const skillLevels = ["lv 2", "lv 3", "lv 4", "lv 5"] as const
...
export const transcendenceSteps = ['1', '2', '3', '4', '4+', '5', '5+', '5++', '6'] as const

export const affinityUpgradeStones = [
...
export const affinityGifts = [
    ...
    {
        items: ['Smartphone', 'Decorative Chest Armor', "Witch's Cauldron", 'Lion Statue', "Phantom Bird's Egg"],
        points: 500,
        bonus: 250
    },
    {
        items: ['Dungeon Core Fragment', "Noble's Ceremonial Sword", 'Magic Textbook', 'Dreamcatcher', 'Leaf of World Tree'],
        points: 1000,
        bonus: 500
    },
] as const
...
export const specialEquipmentMaterials = {
    ee: [
...
export const LABELS = {
    ...
    gemsDesc2: {
        en: "For example, if a DPS unit is lacking Crit Chance, equipping a Crit Chance gem can help reach key thresholds. Gem choices should always align with substat priorities and the character's role.",
        jp: "例えば、DPSユニットがクリティカル率が不足している場合、クリティカル率ジェムを装備することで重要な閾値に到達できます。ジェムの選択は常にサブステータスの優先順位とキャラクターの役割に合わせるべきです。",
        kr: "예를 들어, DPS 유닛에 치명타 확률이 부족하다면 치명타 확률 보석을 장착하여 핵심 임계값에 도달할 수 있습니다. 보석 선택은 항상 서브스탯 우선순위와 캐릭터의 역할에 맞아야 합니다.",
        zh: "例如，如果DPS角色缺少暴击率，装备暴击率宝石可以帮助达到关键阈值。宝石选择应始终与副属性优先级和角色定位保持一致。"
    },
...
// ---- Components
export function UpgradeTable({ lang }: { lang: TenantKey }) {
    return (
                ...
                {upgradeStages.map((stage, idx) => (
                    <tr key={idx} className="align-middle">
                        <td className="pr-2 whitespace-nowrap">
                            <GuideIconInline name={stage.icon} text={lRec(LABELS[stage.stage as keyof typeof LABELS] as LangMap, lang)} />
                        </td>
                        <td className="px-2">
                        ...
                        <td>
                            {stage.cost ? <ItemInlineDisplay names={stage.item} /> : lRec(LABELS.startingStage, lang)}
                        </td>
                    </tr>
                ))}
            </tbody>
...
export function TranscendenceTable({ lang }: { lang: TenantKey }) {
    const getUnlock = (icon: string): ReactNode => {
        switch (icon) {
            ...
            case '4':
                return (
                    <>
                        <StarLevel levelLabel="1" size={14} /> {lRec(LABELS.transcendence4part1, lang).split('while')[0]} <StarLevel levelLabel="2" size={14} /> {lRec(LABELS.transcendence4part1, lang).includes('while') && 'gain self-stat while'} <StarLevel levelLabel="3" size={14} /> {lRec(LABELS.transcendence4part2, lang)}
            ...
            case '6':
                return (
                    <>
                        <StarLevel levelLabel="1" size={14} /> {lang === 'en' ? 'and' : ''} <StarLevel levelLabel="2" size={14} /> {lRec(LABELS.transcendence6part1, lang)}
                        <br /> <StarLevel levelLabel="3" size={14} /> {lRec(LABELS.transcendence6part2, lang)}
...
export function SkillCostTable({ lang }: { lang: TenantKey }) {
    return (
        <table className="table-auto text-sm text-white border-separate border-spacing-y-2">
            <thead>
                <tr>
                    <th className="text-left pr-4">{lRec(LABELS.upgradeLevel, lang)}</th>
            ...
            <tbody>
                {skillLevels.map((lv) => (
                    <tr key={lv}>
                        ...
                        {[1, 2, 3].map((rarity) => {
                            const key = rarity as keyof typeof skillCosts
                            const data = skillCosts[key]?.[lv]
                            return (
...
export function XPItemsList({ lang }: { lang: TenantKey }) {
    return (
        <ul>
            {xpItems.map(({ item, xp }) => (
                <li key={item}>
                    <ItemInlineDisplay names={item} /> {lRec(LABELS.grantsXP, lang)} {xp.toLocaleString()} {lRec(LABELS.xpUnit, lang)}
                </li>
            ))}
        </ul>
    )
...
export function AffinityList({ lang }: { lang: TenantKey }) {
    return (
        <ul>
            {affinityUpgradeStones.map(({ item, points }) => (
                <li key={item}>
                    <ItemInlineDisplay names={item} /> +{points}
                </li>
            ))}
            {affinityGifts.map(({ items, points, bonus }, idx) => (
                <li key={idx}>
                    {items.map(item => (
                        <ItemInlineDisplay key={item} names={item} text={false} />
                    ))}
                    {' '}+{points.toLocaleString()} ({lRec(LABELS.bonus, lang)}: +{bonus})
                </li>
            ))}
        </ul>
    )
...
export function SpecialEquipmentList({ type, lang }: { type: 'ee' | 'talisman'; lang: TenantKey }) {
    ...
    return (
        <ul>
            {materials.map(({ item, range, needed }) => {
                ...
                return (
                    <li key={item}>
                        <ItemInlineDisplay names={item} /> {lRec(LABELS.fromLvTo, lang)} {from} {lRec(LABELS.toLv, lang)} {to} ({needed} {lRec(LABELS.needed, lang)})
                    </li>
                )
            })}
        </ul>
...
export function GearSection({ lang }: { lang: TenantKey }) {
    const points = [
        LABELS.gearPoint1,
        LABELS.gearPoint2,
        LABELS.gearPoint3,
