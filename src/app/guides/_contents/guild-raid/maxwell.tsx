...
type GuildRaidGuideVersion = {
    ...
    phase2: {
        overview: string[]
        teams: Record<string, Phase2Team>  // ⬅️ ici le changement
        video?: {
            id: string
            title: string
        }
...
export default function MaxwellGuildRaidGuide() {
    const data: Record<string, GuildRaidGuideVersion> = guideData
    const versionKeys = Object.keys(data)

    const [selected, setSelected] = useState('v1')
    const version = data[selected]
        ...
        `}
            ...
            {tab === 'phase1' && (
                <div className="flex flex-col md:flex-row gap-8 mb-8">
                    {/* Colonne gauche : Boss images + Geas dessous */}
                        ...
                        <div className="flex flex-col items-center gap-4 mt-4 w-full">
                            {Object.entries(boss.geas).map(([level, entry]) => (
                                <div key={level} className="flex flex-col items-center gap-2 w-full">
                                    ...
                                    {/* Geas groupés horizontalement et centrés */}
                    ...
                    {/* Colonne droite : stratégie et units */}
            ...
            {tab === 'phase2' && (
                <div>
                    ...
                    <TeamTabSelectorWithGeas teams={version.phase2.teams} bosses={version.phase1.bosses} />
                </div>
            )}
        </div>
    )
