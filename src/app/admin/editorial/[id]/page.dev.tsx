...
export default async function AdminEditorialCharacter({
  params,
}: {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-content-strong text-xl font-semibold">
        {characterDisplayName(char)} <span className="text-content-subtle text-sm">({id})</span>
      </h1>
      <EditorialEditor id={id} curated={curated} charNames={charNames} />
    </div>
  );
}
