...
| **Croissance**       | évolution (lignes cumulatives), transcendance (taux/étoile), archive (taux/niveau), éveil (nœuds → stat/buff), monad (nœuds par perso)                            | `CharacterEvolutionStatTemplet`, `CharacterTranscendentTemplet`, `CharacterArchiveStatTemplet`+`ArchiveBonusTemplet`, `CharacterAwakening*`, `MonadGateEnchantNodeTemplet` |
...
débloqués, nœuds monad, niveau d'archive, 6 pièces (item + enchant + main +
