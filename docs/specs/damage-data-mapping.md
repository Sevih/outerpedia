...
| **lien skill → damage templet** | l'affectation FAISANT FOI est dans les AnimationEvents (littéraux chiffrés, spec § 12.4), mais les IDs suivent une **convention de nommage** : `<charId>_Skill_<n>[_<hit>…]` (1392/1542 lignes), ex. `2000001_Skill_2_1..3` ; suffixes de variantes (`_Upgrade`, `_New`, `_A/_B`, `_Burst3`, `_SWORD`/`_KICK`…) | ✅ dérivable par convention (numéro de skill + chaîne de hits ordonnée) ; ⚠️ le CHOIX de variante (upgrade d'ascension, branche A/B) reste à curer/vérifier in-game |
| équivalents monstres            | `MonsterSkillTemplet` (avec `TriggerName`) / `MonsterSkillLevelTemplet` (`DamageFactor`, `WGReduce`, `BuffID`, `Cool`) / `MonsterDamageTemplet` (IDs `<monsterId>_Skill_<n>_<hit>`)                                                                                                                             | ✅ mêmes formes, inventoriées                                                                                                                                       |
...
| sets (2P/4P)             | `ItemSpecialOptionTemplet` : `OptionType_2P`/`_4P` + `StatType`/`ApplyingType`/`OptionValue`, ou buffs ; `BreakLimitCount` ; ligne `IsAdd=false` = remplace, `IsAdd=true` = s'ajoute                                                                                     | ✅                                                                                        |
| équipement exclusif (EE) | `SpecialEquipEnchantTemplet` (coûts) ; l'effet unique est une option principale de type **buff**, résolue au niveau **`enchant + 1`** dans `BuffTemplet` (spec § 17.5)                                                                                                   | ✅                                                                                        |
| artefacts                | `ArtifactTemplet.OptionID` → `BuffTemplet.BuffID` (`bid_artifact_*`) — des buffs ordinaires, comme prévu § 15                                                                                                                                                            | ✅                                                                                        |
