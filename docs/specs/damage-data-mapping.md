...
| **skill → clips d'animation**   | DÉCOUVERT le 22/08/2026 (spec formule § 8.1) : `CharacterSkillTemplet.TriggerName` (CSV ORDONNÉ de triggers d'animator — « Skill2,Skill2_2 » = deux clips) + `TriggerNameSkip` (état SKIP), résolus par le controller compilé `AC_<charId>` des bundles (transitions : hash de condition → `m_TOS` → état → clips) et les `EventAttackStart` des clips (séquence réelle des templets, rejeux compris). Extraction `datagen/damage/extract-anim-events.py` → `anim-events.json` (committé) → `clips.ts` → `DamageSkill.clips` de `characters.json`                          | ✅ 998/1062 skills à hits résolus ; 13 chaînes `clipsUnresolvedChains` + skills sans clips → fallback heuristique § 12.4 du moteur                                                                  |
