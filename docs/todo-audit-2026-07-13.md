...
- [ ] **PRIO (demande Sevih 2026-07-14) — retirer les comparaisons V2 des extracteurs** : l'oracle `data/legacy` a joué son rôle de contrôle de complétude, la dépendance V2 doit sortir. Périmètre à balayer : `datagen/extractor/v2-control.ts` et le contrôle de cohérence, `equipmentV2Control`/`EquipmentReport` (Extractor gear), l'Extractor Effect (contrôle régression V2), les statuts « diff » de la sidebar issus du contrôle V2, `core/diff` de l'extracteur, les lectures `../outerpedia-v2` — **SAUF le regen coupons/banner : on le GARDE tant que la bascule prod V2→V3 n'est pas faite** (décision Sevih 2026-07-14 — la V2 en prod reste la source vivante de ces données éditoriales jusqu'à la bascule). Une fois les contrôles retirés : `data/legacy/` devient supprimable (cf. modèle de travail — snapshot exclu de prettier).
...
- [ ] Branches locales : 3 des 4 élaguées (vérifié 16/07) — reste `backup/site-rebuild` à trancher.
