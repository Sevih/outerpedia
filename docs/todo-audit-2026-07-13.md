...
- [ ] **PRIO (demande Sevih 2026-07-14) — retirer les comparaisons V2 des extracteurs** : l'oracle `data/legacy` a joué son rôle de contrôle de complétude, la dépendance V2 doit sortir. Périmètre à balayer : `datagen/extractor/v2-control.ts` et le contrôle de cohérence, `equipmentV2Control`/`EquipmentReport` (Extractor gear), l'Extractor Effect (contrôle régression V2), les statuts « diff » de la sidebar issus du contrôle V2, `core/diff` de l'extracteur, les lectures `../outerpedia-v2` (regen coupons/banner = à trancher à part : c'est un import ponctuel, pas un contrôle). Une fois les contrôles retirés : `data/legacy/` devient supprimable (cf. modèle de travail — snapshot exclu de prettier).
