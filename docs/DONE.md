...
- **Chrome des guides éditoriaux passé par `t()`, `labels.ts` rendu à la
  ...
  pour un mot (`MAIN_STAT_LABEL` fr « Main Stat » face à « Stat principale »).
  Règle appliquée : `t()` pour le chrome (en-têtes de tableau, onglets,
  libellés d'interface) quand une clé existe DÉJÀ, `labels.ts` pour la prose ;
  aucune clé créée. Les 92 ne se reproduisent pas à l'unité : le balayage
  (`en:` ≤ 60 caractères égal à une valeur de `en.ts`, casse et « : » final
  ignorés) trouve 83 cas dans `general-guides`, avec des comptes par fichier
  identiques à ceux de l'audit (gear 15, beginner-faq 12, timegate 12,
  shop-purchase 10, ether-income 8), plus 15 dans `other/`, plus
  `MAIN_STAT_LABEL` (cité par l'audit, défini dans `gear/index.tsx`) : 99 cas,
  29 migrés, 70 laissés. Le texte anglais rendu ne change nulle part : les
  valeurs EN migrées sont identiques à la lettre. Les autres langues prennent
  le vocabulaire du site : fr « Hebdo » → « Hebdomadaire », « Gold » → « Or »,
  « Details » → « Détails », « Main Stat » → « Stat principale », « Step » →
  « Palier » ; es « Personajes » → « Héroes » ; jp « 日次 » → « デイリー »,
  « 備考 » → « メモ »… `timegate-resource` et `how-to-play` deviennent `async`
  pour `getT` ; dans `ether-income`, les six libellés de chrome ne passent plus
  par `labelKeys` : ils sont posés par `t()` dans le modèle du calculateur. Le
  choix de clé suit l'endroit où la même chose s'affiche déjà : fiche
  équipement (`equip.detail.*`), en-têtes de reset (`home.resets.*`),
  hero-tracker (or, niveau, base/Core Fusion).

  | Migré (fichier · entrée → clé)                                                                                                                  | Laissé (fichier · entrée : raison)                                                                                                                                                                                                                                                                          |
  | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | timegate · `headers.source` → `equip.detail.source` (aussi l'`ariaLabel` des onglets)                                                           | timegate · `badges.guild/shop/craft` : `Record<SourceType>` dont `mission` n'a pas de clé, et `progress.shop.guild` dit « 所属ギルド » (guilde d'appartenance) en jp/kr/zh                                                                                                                                  |
  | timegate · `headers.weekly/monthly` → `home.resets.weekly/monthly`                                                                              | timegate · `sources` guild, arena, stars, survey, worldboss, kates-workshop (6) : noms d'entités du jeu dans une table indexée par `sourceKey`, et le fr garde exprès l'anglais (« Guild Shop ») là où `progress.shop.*` traduit (« Boutique de guilde »). C'est une décision de vocabulaire, pas du chrome |
  | shop-purchase · `colPriority` → `equip.detail.priority`                                                                                         | shop-purchase · `SHOP_TABS` Guild Shop, Event Shop, Joint Challenge, Friendship Point, Arena Shop, World Boss, Adventure License, Survey Hub (8) : même raison, liste de shops alignée sur les clés du générateur                                                                                           |
  | shop-purchase · `colNotes` → `page.character.gear.note`                                                                                         | free-heroes · `thRecommended` : `gear-usage-finder.mode_reco` vaut « 推荐模式 » (mode recommandé) en zh                                                                                                                                                                                                     |
  | free-heroes · `thSource` → `equip.detail.source`                                                                                                | heroes-growth · `tabAffinity` : le jp du guide (親密度) est le terme du jeu (85 occurrences dans `items.json`), la locale dit 好感度, donc la migration dégraderait le jp                                                                                                                                   |
  | free-heroes · `thCharacters` → `nav.characters`                                                                                                 | heroes-growth · `tabGear` : fr/es « Gear » (jargon conservé, CONVENTIONS) contre « Équipement »                                                                                                                                                                                                             |
  | free-heroes · `thDetails` → `guides.rewards.details`                                                                                            | heroes-growth · `grantsXP` (fragment « grants » en minuscule dans une phrase), `talismanTitle` (titre de section éditorial)                                                                                                                                                                                 |
  | heroes-growth · `tabTranscendence` → `page.character.toc.transcend`                                                                             | unlock-content · `base` : homonyme (jp アジト, le repaire, contre « version de base ») ; `Talisman`, `Precise Craft` : `modeName`, override de donnée éditoriale                                                                                                                                            |
  | heroes-growth · `colGold` → `tools.hero-tracker.gold`                                                                                           | gear · `note` : label de Callout (jp 注意), la clé trouvée est le champ note d'un rapport (ノート)                                                                                                                                                                                                          |
  | heroes-growth · `colLevel` → `tools.hero-tracker.level`                                                                                         | gear · `basics_overviewTitle`, `method_reforge_title`, `method_breakthrough_title` : titres de section, et jp 再鍛造/突破 contre 再錬成/限界突破                                                                                                                                                            |
  | gear · `tab_ascension` → `equip.detail.ascension`                                                                                               | gear · `basics_weapon/accessory/armor` : mots insérés dans une phrase                                                                                                                                                                                                                                       |
  | gear · `ascension_activationCost` → `equip.detail.activation_cost`                                                                              | gear · `slot_weapons/accessories/gloves` : termes d'une liste éditoriale dont `slot_otherArmor` n'a pas de clé                                                                                                                                                                                              |
  | gear · `ascensionTable_step` → `equip.detail.step`                                                                                              | banner-mileage · « Festival: », « Seasonal: », « Collab: » : amorces de définition avec deux-points, prose                                                                                                                                                                                                  |
  | gear · `ascensionTable_gold` → `tools.hero-tracker.gold`                                                                                        | ether-income · `guild` : 所属ギルド, comme plus haut ; `variable.coupons` : élément de la liste des sources variables (contenu), casse différente de `tools.coupon-codes`                                                                                                                                   |
  | gear · `ascensionBonus_effect` → `equip.detail.effect`                                                                                          | quirk · `TERMS` counteractStrongEnemies, utility, adventureLicense : termes officiels substitués aux `{ICON_*}` dans la prose ; `tab_guide` : `changelog.type.guide` est un type d'entrée, zh 攻略 contre 指南                                                                                              |
  | gear · `MAIN_STAT_LABEL` (index.tsx) → `page.character.ee.main_stat`                                                                            | beginner-faq · `limited`, `premium`, `exclusiveEquipment`, `accessories`, `armor` : titres de MiniPanel, labels de réponse ou mots dans la prose ; `dps` : rôle d'équipe dont les voisins (buffer, healer) n'ont pas de clé ; `katesWorkshop` : nom d'entité                                                |
  | ether-income · `worldBoss` → `progress.task.world-boss`                                                                                         | beginner-faq · `skillUpRule` + `skillLevel2Weakness`, `effectChanceDuration`, `damageIncreasesDps` : prose, la même règle que `page.character.skill.priority_rule_*` de la fiche perso, dont la locale porte la prose. Le dédoublonnage se ferait dans l'autre sens, hors de la règle du lot                |
  | ether-income · `source` → `equip.detail.source`                                                                                                 | stats · `pr_accordion` « Priority » : la STAT (zh 行动值), homonyme de la colonne                                                                                                                                                                                                                           |
  | ether-income · `daily/weekly/monthly` → `home.resets.*`                                                                                         | daily-stamina · `heading_adventureLicense` : titre de section                                                                                                                                                                                                                                               |
  | ether-income · `notes` → `page.character.gear.note`                                                                                             | other/roadmap-2026-h2 (8) et roadmap-2026 (2) : donnée éditoriale datée (entrées, amorces « Character: », « Story: »…)                                                                                                                                                                                      |
  | core-fusion · `exclusiveEquipment` → `page.character.toc.ee` (valeurs identiques dans les six langues)                                          | other/service-transfer · `applyLabel` « Apply: » (jp 申請期間, période de demande, homonyme du bouton), `overviewTitle` (titre de section)                                                                                                                                                                  |
  | core-fusion · `oldEE`/`newEE` → `tools.hero-tracker.base`/`coreFusion`                                                                          | other/how-to-play · `pcWarningLabel` « Important: » : le deux-points et l'espace fr n'existent pas dans `guides.tips.important`                                                                                                                                                                             |
  | beginner-faq · `onThisPage` → `page.character.toc.on_this_page`                                                                                 |                                                                                                                                                                                                                                                                                                             |
  | other/how-to-play · `sidebarOfficial` → `footer.official_website` ; `LOCALIZED_LINKS.officialwebsite` → `link.officialwebsite` (URL identiques) |                                                                                                                                                                                                                                                                                                             |

  Vérifié : `pnpm typecheck` et `pnpm lint` sans sortie (dernière ligne :
  la commande elle-même), `pnpm test` « Tests 1956 passed (1956) ». Rendu
  contrôlé sur le serveur de dev : `/fr/guides/general-guides/timegate-resource`,
  `ether-income`, `gear`, `free-heroes-start-banner`, `beginner-faq`,
  `/jp/…/heroes-growth` et `/zh/guides/other/how-to-play` affichent les
  valeurs de la locale (« Hebdomadaire », « Palier », « Stat principale »,
  « Détails », « Sur cette page », « 官方网站 »), sans aucune clé brute.
  Aucun autre changement visuel : même balisage, seules les chaînes ont
  changé. Laissé pour une décision de Sevih : aligner les noms de shops et
  de sources (fr anglais dans les guides, traduit dans `progress.shop.*`),
  et le jp de `tools.hero-tracker.affinity` (好感度 contre 親密度, terme du
  jeu). Repéré hors périmètre : `orLabel="or"` et « Lv. 1 » en dur dans
  `core-fusion`, déjà listés au H7.
...
GameLang[]` avait été posé le 07/08 : trois `{ en: '', jp: '', kr: '', zh: '' }`
  ...
  **Non fait, volontairement** (arbitrages Sevih du jour) : le promote (revue
  ...
  tombent au promote. Après le premier promote, deux choses que seule la donnée validée pouvait montrer : les 7 rencontres RETENUES (World Boss Ragnakeus, retirées du jeu) gardaient des dicts à 4 langues — le validé mélangeait deux formes de `LangDict` et `tsc` refusait le JSON committé. `applyRetention` complète désormais à vide, récursivement, les langues apparues depuis (`completeLangDicts`, testé), et `lRec` replie sur l'anglais une chaîne VIDE comme une chaîne absente (`||`, plus `??`, testé) : un blanc n'est pas une traduction. Un second `promote --apply` (7 entités, un seul fichier) clôt le typecheck. La traduction Haiku de `es.ts`, elle, a été lancée et a échoué sur les 30 lots : l'API répond « credit balance is too low » — compte à recharger, script prêt à relancer (cache par lot, rien à repayer deux fois). Relancée par Sevih une fois le compte rechargé : 30 lots, 1 168 valeurs, variables et balises validées ; installée et formatée. La donnée promue a fait coincer le test des cartes sur « Flic de la canicule Delta » (2000121, fr) : nom court `en: "H.Delta"` dans le style des dix existants, les noms courts fr/es restant à curer (TODO). Seconde passe le soir même : une RELECTURE par Haiku (lots de 40, seules les entrées à corriger reviennent, motif à l'appui) a proposé 85 corrections — 64 gardées (accords, tildes, anglicismes, « Nv. » avec point, « Restablecer » pour le bouton de remise à zéro que les deux passes avaient lu comme « terminer »), 21 arbitrées contre la donnée du jeu : le relecteur voulait « personaje » là où le jeu dit « Héroe », des accords au féminin sur des libellés sans nom, « Debilidades » pour Debuff quand la communauté garde le mot, « Limpieza » pour un « clear ». Règle retenue : le vocabulaire officiel (colonne `Spanish`) prime, le jargon partagé par toutes les locales (`builds`, `clear`, `Debuffs`, `Colab`) reste. Rapport complet dans le scratchpad de la session. Noms courts : DÉCISION Sevih le soir même — l'abréviation ANGLAISE pour toutes les langues (« S.Regina » sous le nom français comme sous le japonais), c'est déjà ce que font jp/kr/zh ; les six persos concernés replient donc sur elle, rien à curer, la règle est écrite en tête de `lib/data/short-names.ts`.
